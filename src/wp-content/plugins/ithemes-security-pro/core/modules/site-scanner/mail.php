	...
	public static function format_scan_body( ITSEC_Mail $mail, $scan ) {
		...
		if ( $vulnerabilities && $vulnerabilities->count() ) {
			...
			$mail->add_text(
				...
				' <b>' . esc_html__( 'Either quickly update the vulnerable theme, plugin or WordPress version immediately to the newest version or immediately deactivate the plugin or theme until a fix is available.', 'it-l10n-ithemes-security-pro' ) . '</b>',
				'dark'
			);
